...
namespace TLCGen.Generators.Shared.ProjectGeneration
{
    public class CCOLVisualProjectGenerator
    {
		private bool _prevCondition;
        private string _ccolinclpaths;
        private string _ccolextralibs;
        private string _ccollibsnotig;
        private string _ccollibspath;
        private string _ccolppdefs;
        private string _ccolppdefsextra;
        private string _ccolrespath;
        private string _ccollibs;

        private string HandleFileLine(string line, ITLCGenPlugin plugin)
        {
            var writeline = line;
            ...
            if (writeline.Contains("__"))
            {
                ...
                var actualccollibspath = _ccollibspath == null ? "" : _ccollibspath.Remove(_ccollibspath.Length - 1);
                if(!actualccollibspath.EndsWith("\\"))
                {
                    actualccollibspath += "\\";
                }
                writeline = writeline.Replace("__CCOLLIBSDIR__", actualccollibspath);
                writeline = writeline.Replace("__CCOLLIBSDIRNOTIG__", _ccollibsnotig ?? "");
                writeline = writeline.Replace("__CCOLLIBS__", _ccollibs);
                writeline = writeline.Replace("__CCOLLIBSEXTRA__", _ccolextralibs ?? "");
                
                var actualccolrespath = _ccolrespath == null ? "" : _ccolrespath.Remove(_ccolrespath.Length - 1);
                ...
                writeline = writeline.Replace("__PREPROCESSORDEFS__", _ccolppdefs);
                writeline = writeline.Replace("__PREPROCESSORDEFSEXTRA__", _ccolppdefsextra);
            }
        ...
        public static List<string> GetNeededPreprocDefs(ControllerModel c)
        {
        ...
        public static List<string> GetNeededCCOLLibraries(ControllerModel c, bool automaat, int visualVer)
        {
            ...
            var neededlibs = new List<string> {"ccolreg.lib", (c.Data.GeneratorType == "Traffick" ? "mlfunc.lib" : "lwmlfunc.lib"), "stdfunc.lib"};
            if (!automaat)
            {
        ...
        public string GenerateVisualStudioProjectFiles(ITLCGenPlugin plugin, string templateFolder, string templateName, int visualVer, string outputfilename, CCOLGeneratorVisualSettingsModel settings)
        {
            var sb = new StringBuilder();

            var templatefilename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"Settings\\{templateFolder}\\{templateName}.xml");
	        var filtersfilename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"Settings\\{templateFolder}\\{templateName}_filters.xml");
            
            _ccolinclpaths = settings.CCOLIncludesPaden;
            _ccolextralibs = settings.CCOLLibs;
            _ccollibsnotig = settings.CCOLLibsPathNoTig;
            _ccollibspath = settings.CCOLLibsPath;
            _ccolppdefs = string.Join(";", GetNeededPreprocDefs(plugin.Controller).OrderBy(x => x));
            _ccolppdefsextra = settings.CCOLPreprocessorDefinitions;
            _ccolrespath = settings.CCOLResPath;
            _ccollibs = string.Join(";", GetNeededCCOLLibraries(plugin.Controller, false, visualVer).OrderBy(x => x));
            ...
            if (File.Exists(templatefilename))
            {
                ...
                {
                    var writeline = HandleFileLine(line, plugin);
